<template>
	<view class="content">
		<view v-for="(item,index) in homePosts" :key="index" class="card">
			<uni-card>
			<navigator
				:url="'/managePages/examine/examineDetail/examineDetail?examineId='+ item.examineId + '&type='+ item.type +'&fromPage=examToDo'">
	...
	export default {
		...
		methods: {
			getHomePosts() {
				let _this = this;
				uni.request({
					url: _this.apiServer + "/api/get/getAll/examToDo",
					header: {
